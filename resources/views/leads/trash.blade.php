                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="bootstrap-select fm-cmp-mg">
                                            <select class="form-control user_id" name="user_id" id="user_id">
                                                ...
                                                ?>
                                                @include('partials.allocation-options', ['users' => $allocation ?? $users, 'selected' => $user_id])
                                            </select>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="col-md-3">
                    <div class="count-uncheckboxes-wrapper" style="display: inline-flex;">
                        @if(Auth::user()->can('trashed.lead.share'))
                            <a style="margin: 0 10px;" rel="tooltip" href="#" data-original-title="" title="Share-Lead" data-toggle="modal" data-target="#multileadshare" id="multi_shareLead">
                                <i class="fa fa-share-alt" aria-hidden="true"></i>
                            </a>
                        @endif
                        @if(Auth::user()->can('trashed.lead.transfer'))
                            <a style="margin: 0 10px;" rel="tooltip" href="#" data-original-title="" title="Transfer-Lead" data-toggle="modal" data-target="#multileadtransfer" id="multi_transferLead">
                                <i class="fa fa-share" aria-hidden="true"></i>
                            </a>
                        @endif
                        @if(Auth::user()->can('trashed.lead.delete'))
                            <form action="{{ url('multi/leads/delete/permanently') }}" method="post" style="margin: 0 10px;" id="multiDeleteLeadForm">
                                @csrf
                                <input type="hidden" name="multi_del_lead_ids" id="multi_del_lead_ids" value="">
                                <button type="button" class="fa fa-trash" style="background: none; border: none; color: inherit; cursor: pointer; padding: 0;" data-original-title="" title="Delete Data" onclick="leadsPermanentlyDeleteConfirm(event);"></button>
                            </form>
                        @endif
                    </div>
                </div>
                ...
                <table class="table ps-table count_form_checkboxes" id="tbl">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if(Auth::user()->can('trashed.lead.share') || Auth::user()->can('trashed.lead.transfer') || Auth::user()->can('trashed.lead.delete'))
                                <td>
                                ...
                                <td >
                                    ...
                                    <div style="text-align: center">
                                        ...
                                        <a  rel="tooltip" href="#" data-original-title="" title="" data-toggle="modal" data-bind="<?php echo $lead->id; ?>" data-client="<?php echo $lead->client_id; ?>" itemid="<?php echo $item_id; ?>" data-target="#tasklog" id="getTask">
                                            <i class="fa fa-calendar mr-15" aria-hidden="true"></i>
                                        </a>
                                        ...
                                        <div style="display: inline-flex; margin-left: 5px;">
                                            @if(Auth::user()->can('trashed.lead.restore'))
                                                <form action="{{ url('lead/restore', $lead->id) }}" method="post" class="mr-10">
                                                    @csrf
                                                    <button type="button" class="fa fa-undo mr-5" data-original-title="" title="Actice Data" onclick="confirm('{{ __("Are you sure you want to Active this Record?") }}') ? this.parentElement.submit() : ''"></button>
                                                </form>
                                            @endif
                                            @if(Auth::user()->can('trashed.lead.delete'))
