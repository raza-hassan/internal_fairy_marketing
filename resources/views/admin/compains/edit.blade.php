...
<div class="ps-main__wrapper">
    ...
    <section class="ps-new-item">
        <form method="post" action="{{url('admin/compain',$compain->id) }}" class="form-horizontal"  id="leadForm">
            @csrf
            @method('PUT')
                            ...
                            <figcaption>Comapin Info</figcaption>
                                <div class="ps-block__content">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control name"  name="compain_name" type="text" placeholder="{{ __('Comapin Name') }}" value="{{ old('compain_name', $compain->compain_name) }}" required=""/>
                                    </div>
                                        ...
                                        <select class="ps-select {{ $errors->has('office_id') ? ' is-invalid' : '' }} office_id" id="input-office_id"  title="office Name" name="office_id" required="true" aria-required="true">
                                            <option selected disabled value="">Select Office</option>
                                                @foreach($offices as $office)
                                                    <option value="{{$office->id}}" {{ $office->id==$compain->office_id ? 'selected' : '' }} @if(old('office_id')==$office->id ) selected  @endif >{{$office->name}}</option>
                                                @endforeach
                                        </select>
                                        ...
                                        <select class="ps-select {{ $errors->has('project_id') ? ' is-invalid' : '' }} project_id" id="input-project_id"  title="Project Name" name="project_id" required="true" aria-required="true">
                                            <option selected disabled value="">Select project</option>
                                                @foreach($projects as $project)
                                                    <option value="{{$project->id}}" {{ $project->id==$compain->project_id ? 'selected' : '' }} @if(old('project_id')==$project->id ) selected  @endif >{{$project->name}}</option>
                                                @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control form_id"  name="form_id" type="text" placeholder="{{ __('Form Name (ID)') }}" value="{{ old('form_id' , $compain->form_id) }}" required=""/>
                                    </div>
