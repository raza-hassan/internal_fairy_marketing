...
<div class="ps-main__wrapper">
    ...
    <header class="header--dashboard">
        ...
        <div class="header__right">
            <a class="ps-btn success" href="{{ url('admin/compain/create') }}"><i class="icon icon-plus mr-2"></i>Add New </a>
        </div>
    </header>
                    ...
                    @foreach($compains as $record)
                        <tr class="gradeX">
                            ...
                            <td class="td-actions text-right ">
                                <form method="post" action="{{ url('admin/compain',$record->id)}}" >
                                    @csrf
                                    @method('DELETE')
                                    <a rel="tooltip" class="fa fa-pencil mr-5 " title="Edit Data" href="{{ url('admin/compain/'.$record->id.'/edit') }}" data-original-title="" ></a>
                                    ...
                                    @if($record->status==1)
                                        <a rel="tooltip" title="Inactive"
                                            href="{{ url('admin/compain/inactive/'.$record->id) }}" data-original-title="" >
                                            <i class='fas fa-toggle-on' style='font-size:20px;'></i>
                                        </a>
                                    @else
                                        <a rel="tooltip"  title="Active"
                                            href="{{ url('admin/compain/active/'.$record->id) }}" data-original-title="" >
