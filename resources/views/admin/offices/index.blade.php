...
<div class="ps-main__wrapper">
    ...
    <header class="header--dashboard">
        ...
        <div class="header__right">
            <a class="ps-btn success" href="{{ url('admin/offices/create') }}"><i class="icon icon-plus mr-2"></i>Add New </a>
        </div>
    </header>
    <section class="ps-items-listing">
        ...
        <div class="ps-section__content">
            <div class="table-responsive">
                ...
                <tbody>
                    @foreach($Offices as $record)
                        <tr class="gradeX">
                            <td> {{ $record->id }} </td>
                            <td> {{ $record->name }} </td>
                            <td class="td-actions text-right ">
                                <form action="{{ url('admin/offices/destroy', $record) }}" method="post">
                                    @csrf
                                    @method('delete')
                                    <a rel="tooltip" class="fa fa-pencil mr-5 " title="Edit Data" href="{{ url('admin/offices/edit', $record) }}" data-original-title="" ></a>
